  ...
  select    rc.recipe_id,
            ...
            comp_amount
            ||  case
                    when rc.comp_amount_upper is not null
                    then '-' || comp_amount_upper
                end
            ||  'x '
            ||  case
                    ...
                    then    case
                                when rc.comp_level is not null
                                then 'niv. ' || rc.comp_level || ' '
                            end
                            || it.item_type_name
                            || ' ('
                            || it.component_type
