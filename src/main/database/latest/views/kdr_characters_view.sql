  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW KDR_CHARACTERS_VIEW as
    ...
    select ch.char_id,
           ...
           case
               ...
               when char_soul_left between 1 and 33   then
                   '<i class=fas fa-2x fa-battery-quarter></i>'
               when char_soul_left between 34 and 66  then
                   '<i class=fas fa-2x fa-battery-half></i>'
               when char_soul_left between 67 and 99  then
                   '<i class=fas fa-2x fa-battery-three-quarters></i>'
               when char_soul_left = 100              then
                   '<i class=fas fa-2x fa-battery-full></i>'
           ...
           get_total_xp_spend_skills(
               ch.char_id
           )                                              as xp_spend_skills,
           get_total_xp_spend_spells(
               ch.char_id
           )                                              as xp_spend_spells,
           get_total_xp_spend_prayers(
               ch.char_id
           )                                              as xp_spend_prayers,
           get_total_mana_divinity_gain(
               ch.char_id
           )                                              as total_mana_divinity_gain
