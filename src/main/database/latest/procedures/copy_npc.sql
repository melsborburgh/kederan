create or replace procedure copy_npc(i_npc_id kdr_npcs.npc_id%type, i_npc_name kdr_npcs.npc_name%type default null)
is
...
begin
    ...
    begin
        for rec in (select * from kdr_npc_skills where npc_id = i_npc_id) loop
    ...
    begin
        for rec in (select * from kdr_npc_spells where npc_id = i_npc_id) loop
    ...
    begin
        for rec in (select * from kdr_npc_prayers where npc_id = i_npc_id) loop
